...
export interface Feedback {
  ...
  tags: string[];
  comment: string;
  date: string; // ISO string date of feedback submission
}
...
export type ViewMode = 'cards' | 'list' | 'calendar' | 'timeline' | 'week';

export interface ActiveFilters {
    status: Visit['status'][];
    locationType: Visit['locationType'][];
    hostStatus: ('assigned' | 'unassigned' | 'not_needed')[];
    speakerTags: string[];
    hostTags: string[];
}
...
export type SpecialDateType = 'assembly' | 'co_visit' | 'special_talk' | 'no_meeting' | 'other';
