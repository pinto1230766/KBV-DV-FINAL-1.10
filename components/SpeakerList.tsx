...
const AISuggestionCriteriaModal: React.FC<{
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (months: number | null) => void;
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        const numMonths = months.trim() === '' ? null : parseInt(months, 10);
        if (numMonths !== null && (isNaN(numMonths) || numMonths < 0)) {
            return;
        }
        onSubmit(numMonths);
    };
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" onClick={onClose}>
            <div className="bg-card-light dark:bg-card-dark rounded-xl shadow-xl w-full sm:max-w-md animate-fade-in-up" onClick={e => e.stopPropagation()}>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="p-6 space-y-4">
                        <div>
                            <label htmlFor="months-input" className="block text-sm font-medium text-text-muted dark:text-text-muted-dark">
                                Dernière visite il y a plus de (mois)
                            </label>
                            <input
                                id="months-input"
                                type="number"
                    ...
                    <div className="bg-gray-50 dark:bg-background-dark px-6 py-4 flex justify-end items-center border-t border-border-light dark:border-border-dark rounded-b-xl">
                        <button type="button" onClick={onClose} className="px-4 py-2 mr-3 bg-card-light dark:bg-card-dark border border-gray-300 dark:border-border-dark rounded-md text-sm font-medium text-text-main dark:text-text-main-dark hover:bg-gray-50 dark:hover:bg-primary-light/20">
                            Annuler
                        </button>
                        <button type="submit" className="px-4 py-2 bg-primary text-white font-semibold rounded-lg hover:bg-primary-light flex items-center gap-2">
...
export const SpeakerList: React.FC<SpeakerListProps> = ({ onSchedule, onAddSpeaker, onEditSpeaker, onSendMessage, isExpanded, onToggleExpand }) => {
    ...
    const handleSuggestSpeakers = async (minMonths: number | null) => {
        if (!isOnline) {
        ...
        try {
            ...
            const criteriaText = minMonths !== null
                ? `Un critère important est que les orateurs suggérés ne doivent pas être venus depuis au moins ${minMonths} mois. Ignore ceux qui sont venus plus récemment.`
    ...
    return (
        <div className="bg-card-light/80 dark:bg-card-dark/80 backdrop-blur-sm rounded-xl shadow-soft-lg p-6 mt-8">
                        ...
                        <div className="flex flex-col md:flex-row gap-4 flex-grow w-full md:w-auto">
                            <div className="relative flex-grow">
                                <input
                                    ...
                                    className="w-full pl-10 pr-4 py-2 border border-border-light dark:border-border-dark rounded-lg focus:ring-secondary focus:border-secondary bg-card-light dark:bg-card-light/10"
                             ...
                             <div className="flex-grow md:flex-grow-0 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="relative">
                                    <select
                                        value={selectedCongregation}
                                        onChange={(e) => setSelectedCongregation(e.target.value)}
                                        className="w-full pl-3 pr-10 py-2 border border-border-light dark:border-border-dark rounded-lg focus:ring-secondary focus:border-secondary appearance-none bg-card-light dark:bg-card-light/10"
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-2 mt-4">
                                        <button
                                            ...
                                            className="py-2 px-3 text-text-muted dark:text-text-muted-dark hover:text-primary dark:hover:text-white rounded-lg hover:bg-gray-100 dark:hover:bg-primary-light/20 transition-colors active:scale-95 font-semibold"
                                            aria-label={`Détails pour ${speaker.nom}`}
                                        >
                                            Détails
                                        </button>
                                         <button
                                            onClick={() => onSendMessage(speaker)}
                                            className="py-2 px-3 text-text-muted dark:text-text-muted-dark hover:text-primary dark:hover:text-white rounded-lg hover:bg-gray-100 dark:hover:bg-primary-light/20 transition-colors active:scale-95"
                                            aria-label={`Envoyer un message à ${speaker.nom}`}
                                        >
            ...
            />
            <AISuggestionCriteriaModal
                isOpen={isCriteriaModalOpen}
                onClose={() => setIsCriteriaModalOpen(false)}
                onSubmit={(months) => {
                    setIsCriteriaModalOpen(false);
                    handleSuggestSpeakers(months);
                }}
            />
        </div>
    );
