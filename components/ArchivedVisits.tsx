...
import { SearchIcon, TrashIcon, CalendarIcon, StarIcon } from './Icons';
import { useData } from '../contexts/DataContext';
import { useConfirm } from '../contexts/ConfirmContext';

interface ArchivedVisitsProps {
    onLeaveFeedback: (visit: Visit) => void;
}

export const ArchivedVisits: React.FC<ArchivedVisitsProps> = ({ onLeaveFeedback }) => {
        ...
        <div>
            ...
            {sortedAndFilteredVisits.length > 0 ? (
                <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                    {sortedAndFilteredVisits.map((visit, index) => (
                        <div 
                            key={visit.visitId} 
                        ...
                        >
                            <div>
                             ...
                             <div className="flex items-center gap-2 self-start sm:self-center">
                                {visit.feedback ? (
                                    <div className="flex items-center gap-1 px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-300">
                                    ...
                                    <button
                                        ...
                                        className="px-3 py-1 bg-primary/10 hover:bg-primary/20 text-primary dark:text-primary-light font-semibold rounded-lg text-xs"
                                    >
                                        Laisser un avis
                                    </button>
                                )}
                                <button 
                                    ...
                                    className="p-2 text-gray-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-full transition-colors active:scale-90"
