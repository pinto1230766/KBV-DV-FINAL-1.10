...
const TalkCard: FC<TalkCardProps> = ({ talk, onEdit, onAssign }) => {
    ...
    const currentStatus = statusInfo[talk.status as keyof typeof statusInfo];

    return (
        <div className="bg-gray-50 dark:bg-card-dark rounded-lg transition-shadow hover:shadow-md">
            <button 
                type="button"
                className="w-full text-left p-3 flex items-center gap-4 focus:outline-none focus:ring-2 focus:ring-primary/50 rounded-lg" 
                onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                ...
                aria-controls={`talk-content-${talk.number}`}
            >
                ...
                <div className="flex items-center flex-shrink-0">
                    ...
                    <ChevronDownIcon 
                        className={`w-6 h-6 text-text-muted dark:text-text-muted-dark transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`} 
                        aria-hidden="true"
                    />
                </div>
            </button>
                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="text-left text-xs text-text-muted dark:text-text-muted-dark">
                                            ...
                                            <th scope="col" className="p-2">Orateur</th>
