                ...
                <div className="p-6 space-y-4 overflow-y-auto flex-1 min-h-0">
                    ...
                    <div>
                        ...
                        >
                            ...
                            {speakers.map(s => {
                                const isUnavailable = visitDate ? visits.some(v => v.visitDate === visitDate && v.id === s.id && v.status !== 'cancelled') : false;
                                return (
                                    <option key={s.id} value={s.id} disabled={isUnavailable} className="disabled:text-red-400 dark:disabled:text-red-600 disabled:bg-gray-200">
                                        {s.nom} ({s.congregation}) {isUnavailable ? '(Déjà programmé)' : ''}
                                    </option>
                                );
                            })}
                        </select>
