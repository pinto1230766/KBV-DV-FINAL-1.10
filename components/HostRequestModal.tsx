...
export const HostRequestModal: React.FC<HostRequestModalProps> = ({ isOpen, onClose, visits, language, onLanguageChange }) => {
  ...
  const generateMessage = useCallback((templateText: string) => {
    const visitList = visits
      .map(v => `- ${new Date(v.visitDate + 'T00:00:00').toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })} : Frère ${v.nom}`)
      .join('\n');
  ...
  const handleShare = async () => {
    if (Capacitor.isNativePlatform()) {
      try {
        ...
        addToast('Message partagé avec succès', 'success');
      } catch (error) {
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex items-center gap-2">
                  {isCustom && (
                    <button type="button" onClick={handleRestoreDefault} className="text-xs font-semibold text-red-600 dark:text-red-400 flex items-center gap-1">
                  ...
                  <button type="button" onClick={handleSaveTemplate} className="px-3 py-1 text-xs bg-primary text-white rounded-md flex items-center gap-1">
