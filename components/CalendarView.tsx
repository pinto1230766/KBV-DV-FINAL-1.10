...
import { Visit, Host, SpecialDate, SpecialDateType } from '../types';
import { ChevronLeftIcon, ChevronRightIcon, EditIcon, HomeIcon, VideoCameraIcon, PrintIcon, ExclamationTriangleIcon, CalendarIcon, UserIcon, WifiIcon, PodiumIcon, ShieldCheckIcon, SparklesIcon, XCircleIcon, InformationCircleIcon } from './Icons';
...
const specialDateInfo: Record<SpecialDateType, { label: string; icon: React.FC<any>; color: string }> = {
    assembly: { label: "Assemblée", icon: PodiumIcon, color: 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900/50 dark:text-indigo-200' },
    co_visit: { label: "Visite du surveillant", icon: ShieldCheckIcon, color: 'bg-sky-100 text-sky-800 dark:bg-sky-900/50 dark:text-sky-200' },
    special_talk: { label: "Discours spécial", icon: SparklesIcon, color: 'bg-purple-100 text-purple-800 dark:bg-purple-900/50 dark:text-purple-200' },
    no_meeting: { label: "Pas de réunion", icon: XCircleIcon, color: 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-200' },
    other: { label: "Autre", icon: InformationCircleIcon, color: 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200' },
};
...
export const CalendarView: React.FC<CalendarViewProps> = ({ onEditVisit }) => {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            const weekDetails: DayDetails[] = weekDates.map(day => {
                ...
                return {
                    ...
                    specialDate: specialDateMap.get(dateStr) || null,
                };
            });
    ...
    }, [calendarGridDates, visits, archivedVisits, hosts, specialDates, currentDate, archivedVisitIds]);
        ...
        <div className="bg-card-light dark:bg-card-dark rounded-xl shadow-lg p-4 sm:p-6">
            ...
            <div className="grid grid-cols-7 grid-rows-6 gap-px bg-border-light dark:bg-border-dark border border-border-light dark:border-border-dark rounded-lg overflow-hidden">
                {calendarGridDetails.map((dayDetails) => {
                    const { date, visit, stays, holidays, vacations, unavailableHosts, specialDate } = dayDetails;
                    ...
                    return (
                        <div key={date.toString()} onClick={() => handleDayClick(dayDetails)} className={cellClasses}>
                            ...
                            <div className="mt-1 space-y-0.5 overflow-hidden flex-grow relative">
                                {specialDate && (
                                    <div className={`p-1 text-xs font-bold leading-tight truncate rounded-md flex items-center gap-1.5 ${specialDateInfo[specialDate.type].color}`} title={specialDate.name}>
                                        {SpecialDateIcon && <SpecialDateIcon className="w-3 h-3 flex-shrink-0" />}
                                        <span className="truncate">{specialDate.name}</span>
                                    </div>
                                )}

                                {stays.map(stay => (
                                    <div key={stay.visit.visitId} style={{ top: `${specialDate ? 1.75 + (stay.track * 1.35) : stay.track * 1.35}rem`}} className={`absolute left-0 right-0 h-5 text-xs font-bold leading-tight flex items-center gap-1 z-0 ${stay.color} ${stay.textColor} ${stay.isStart ? 'rounded-l-md' : ''} ${stay.isEnd ? 'rounded-r-md' : ''}`} title={stay.visit.nom}>
                                        {stay.showText && (
                                            <>
                                ...
                                {visit && !stays.some(s => s.visit.visitId === visit.visitId) && (
                                    ...
                                    >
                                        <div className="flex items-center">
                                            ...
                                            {visit.locationType === 'physical' && <span title="Présentiel"><HomeIcon className="w-3 h-3" /></span>}
                                            {visit.locationType === 'zoom' && <span title="Zoom"><VideoCameraIcon className="w-3 h-3" /></span>}
                        ...
                        <div className="bg-card-light dark:bg-card-dark rounded-lg shadow-xl w-full sm:max-w-md max-h-[90vh] flex flex-col animate-fade-in-up" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="p-6 space-y-6 overflow-y-auto">
                               {uniqueVisits.length > 0 && (
                                 ...
                                 {(selectedDay.vacations.length > 0 || selectedDay.holidays.length > 0 || selectedDay.specialDate) && (
                                    <ModalSection title="Événements" icon={CalendarIcon}>
                                        {selectedDay.specialDate && (() => {
                                            const info = specialDateInfo[selectedDay.specialDate.type];
                                            return (
                                                 <div className={`p-3 rounded-md mb-2 ${info.color}`}>
                                                     <p className="font-bold flex items-center gap-2"><info.icon className="w-5 h-5"/>{info.label}: {selectedDay.specialDate.name}</p>
                                                     {selectedDay.specialDate.description && <p className="text-sm italic mt-1">« {selectedDay.specialDate.description} »</p>}
                                                </div>
                                            );
                                        })()}
                               ...
                               {uniqueVisits.length === 0 && selectedDay.holidays.length === 0 && selectedDay.vacations.length === 0 && !selectedDay.specialDate && selectedDay.unavailableHosts.length === 0 && (
